...
import { FieldDescription } from '../../lib/decorators/FieldDescription';
 ...
 */
function getMetadataFromObject(
  obj: any,
...
export function getOptions(obj: any): ValidationOptions | undefined {
  const retVal = Reflect.getMetadata(MetadataKeys.Fields, obj.prototype);
  if (!retVal) {
    return;
  }
...
export function getGlobalArgs(obj: any): SchemaArgs | undefined {
  const retVal = Reflect.getMetadata(MetadataKeys.Fields, obj.prototype);
  if (!retVal) {
    return;
  }
